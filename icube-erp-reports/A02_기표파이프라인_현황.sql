...
/*==============================================================================================
  ...
  1) **전표 연결은 `DOCU_DT + DOCU_SQ` 한 쌍만 본다.** 한 마감건이 여러 전표로 분개되거나
     여러 마감건이 한 전표로 묶이는 사이트에서는 1:1 로 보이지 않는다. 미연결로 잡힌 건이
     실제로는 다른 전표에 합산되어 있을 수 있다.

  2) **수금은 `RCPAM_FG='0'`(영업모듈)만 집계한다.** 타 모듈에서 입력한 수금은 이 파이프라인에
     나타나지 않는다. S-04·S-06·E-01·A-05 와 같은 기준이므로 그쪽 숫자와는 대사되지만,
     전사 입금 총액과는 다르다.

  3) **`LPURCLS` 는 명세서 누락 테이블**이다. 없는 사이트에서는 매입 파이프라인 구간이 통째로
     빠진다. 실행 로그에 `[WARN] LPURCLS 없음` 이 찍히는지 먼저 볼 것.

  4) **장부반영 판정은 전표 상태(`DOCU_ST`)에 의존한다.** 상태값의 의미가 사이트마다 달라
     (UDR 0/1/2 ↔ API 0/1), 분포를 먼저 확인하지 않으면 미반영 건수가 과대·과소로 나온다.

  5) **금액이 맞는지만 보고 계정과목이 맞는지는 보지 않는다.** 차대가 맞아도 엉뚱한 계정에
     분개된 전표는 여기서 걸러지지 않는다.
